...
const Avatar = ({ user, size = 10 }) => {
  ...
  return (
    <div className={`w-${size} h-${size} ${color} rounded-full flex items-center justify-center text-white font-bold flex-shrink-0`}
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-6">
        <div className="flex flex-col sm:flex-row gap-5">
          <div className="relative">
            ...
            {pro.verified && (
              <div className="absolute bottom-0 right-0 bg-cyan-400 rounded-full p-1 border-2 border-white">
                <I.Check />
              </div>
            )}
          </div>
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-2 mb-1">
              ...
              {pro.verified && <span className="text-xs bg-teal-100 text-teal-700 font-bold px-2 py-0.5 rounded-full flex items-center gap-1"><I.Shield />Verificado</span>}
            </div>
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-2">
              <I.MapPin />{pro.bairro || pro.city || "Saquarema"} — Saquarema/RJ
              <span className="text-gray-300">·</span>
              <I.Clock />{pro.responseTime}
              <span className="text-gray-300">·</span>
              Membro desde {pro.memberSince}
            </div>
            <div className="flex items-center gap-2 mb-3">
              <div className="flex">{stars(pro.rating, "base")}</div>
              <span className="font-bold text-gray-900">{pro.rating}</span>
              <span className="text-gray-500 text-sm">({pro.reviewCount} avaliações)</span>
              <span className="text-gray-300">·</span>
              <span className="text-sm text-gray-500">{pro.completedJobs} serviços concluídos</span>
            </div>
            <div className="flex flex-wrap gap-2">
              {pro.categories?.map(c => {
                const cat = CATEGORIES.find(x => x.id === c);
                return cat ? <span key={c} className="bg-teal-50 text-teal-700 px-3 py-1 rounded-full text-sm font-medium">{cat.emoji} {cat.label}</span> : null;
              })}
            </div>
          </div>
          <div className="flex flex-col gap-2">
            {pro.phone && (
              <button onClick={() => openWhatsApp(pro.phone, "serviço")} className="flex items-center gap-2 bg-cyan-400 hover:bg-teal-700 text-white font-bold px-4 py-2.5 rounded-xl text-sm transition-colors">
                📱 WhatsApp
              </button>
            )}
            {user && user.id !== pro.id && (
              <button onClick={() => setShowReviewModal(true)} className="flex items-center gap-2 bg-yellow-50 hover:bg-yellow-100 text-yellow-700 font-bold px-4 py-2.5 rounded-xl text-sm border border-yellow-200 transition-colors">
                ★ Avaliar
              </button>
            )}
          </div>
        ...
        {pro.bio && (
          <div className="mt-5 pt-5 border-t border-gray-100">
            <h3 className="font-bold text-gray-800 mb-2">Sobre mim</h3>
      ...
      {pro.portfolio?.length > 0 && (
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm mb-6">
          <h2 className="font-black text-gray-900 mb-4">🖼️ Portfólio</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {pro.portfolio.map(p => (
              <div key={p.id} className="border border-gray-200 rounded-xl overflow-hidden">
                <div className="h-32 bg-gradient-to-br from-teal-100 to-cyan-200 flex items-center justify-center text-4xl">💼</div>
                <div className="p-3">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
        <h2 className="font-black text-gray-900 mb-4">⭐ Avaliações ({proReviews.length})</h2>
        {proReviews.length === 0 ? (
      ...
      <div className="flex bg-gray-100 rounded-xl p-1 gap-1 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            ...
            {t.label}{t.count !== null && <span className="bg-gray-200 text-gray-600 text-xs px-1.5 py-0.5 rounded-full">{t.count}</span>}
          </button>
        ))}
      </div>
