...
// Rate limiter simples em memória — previne brute-force no frontend
      ...
      <div className="p-6 sm:p-8 text-center">
        ...
        <div className="flex flex-col gap-3">
          <button onClick={onGoVerify}
            className="w-full bg-teal-700 hover:bg-teal-800 text-white font-black py-4 rounded-xl transition-colors">
...
const MsgPopup = ({ popup, onClose, onOpen }) => {
  ...
  const typeConfig = {
    message:           { icon: "💬", accent: "bg-teal-500",  label: "Nova mensagem"   },
    ...
    <nav className="bg-white border-b border-gray-200 sticky top-0 z-40 shadow-sm">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          {user && (
            <button onClick={() => setPage("messages")} className="hidden sm:flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-teal-700 px-3 py-2 rounded-xl hover:bg-teal-50 transition-colors">
                          ...
                          return (
                            <div key={n.id}
                              ...
                              className={`px-4 py-3 text-sm border-b border-gray-50 flex items-start gap-3 transition-colors
                                ${!n.read ? "bg-teal-50 hover:bg-teal-100" : "hover:bg-gray-50"}
                                ${n.link ? "cursor-pointer" : "cursor-default"}`}>
                              {/* Indicador não lida */}
                              <div className="flex flex-col items-center gap-1 shrink-0 pt-0.5">
                                <span className="text-base">{typeIcon}</span>
                                {!n.read && <span className="w-1.5 h-1.5 bg-teal-500 rounded-full"></span>}
                              </div>
                              <div className="flex-1 min-w-0">
                    ...
                    {unread > 0 && (
                      <div className="p-2 border-t border-gray-100 shrink-0">
                        <button
                          ...
                          className="w-full text-xs text-teal-600 hover:text-teal-800 font-semibold py-1.5 rounded-lg hover:bg-teal-50 transition-colors">
      ...
      <section className="bg-gradient-to-br from-teal-800 via-teal-700 to-cyan-400 text-white overflow-hidden relative">
        ...
        <div className="max-w-6xl mx-auto px-4 py-12 sm:py-20 text-center relative z-10">
          <div className="inline-flex items-center gap-2 bg-white/20 rounded-full px-4 py-1.5 text-sm font-semibold mb-6 backdrop-blur-sm border border-white/30">
            ✨ Plataforma #1 de serviços em Saquarema/RJ
          </div>
            ...
            {user ? (
              <>
                <button onClick={() => setPage(user.role === "CANDIDATE" ? "services" : "request-service")}
                  className="bg-white text-teal-800 font-black px-8 py-4 rounded-2xl text-lg hover:bg-teal-50 transition-all shadow-xl hover:-translate-y-1">
                  {user.role === "CANDIDATE" ? "🔍 Ver Oportunidades" : "📋 Solicitar Serviço"}
                </button>
                <button onClick={() => setPage("dashboard")}
                  className="bg-teal-900/60 hover:bg-teal-900/80 border border-cyan-400/50 text-white font-bold px-8 py-4 rounded-2xl text-lg transition-all backdrop-blur-sm">
                  👤 Meu Painel
                </button>
              </>
            ) : (
      ...
      <section className="max-w-6xl mx-auto px-4 py-12">
        ...
        {db.users.filter(u => u.role === "CANDIDATE").length === 0 ? (
          <div className="text-center py-12 text-gray-400">
            ...
            <button onClick={() => setPage("register")} className="mt-3 text-teal-600 font-bold hover:underline text-sm">Seja o primeiro →</button>
          </div>
        ) : (
...
const ProfilePage = () => {
  ...
  if (!pro) return <div className="p-8 text-center text-gray-400">Nenhum perfil selecionado.</div>;
        ...
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-6 flex flex-col items-center text-center gap-3">
          ...
          {pro.phone && (
            <button onClick={() => openWhatsApp(pro.phone, "serviço")}
              className="flex items-center gap-2 bg-green-500 hover:bg-green-600 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors shadow-sm mt-1">
              📱 WhatsApp
            </button>
          )}
          {proReviews.length > 0 && (
    ...
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        ...
        {user && (user.role === "COMPANY" || user.role === "CLIENT") && (
          <button onClick={() => setPage("request-service")}
            className="flex items-center gap-2 bg-teal-700 hover:bg-teal-800 text-white font-bold px-4 py-3 rounded-xl text-sm transition-colors shadow-sm flex-shrink-0">
      ...
      <div className="flex flex-col gap-3 mb-6">
        ...
        <div className="flex gap-3">
          <select value={cat} onChange={e => setCat(e.target.value)} className="flex-1 border border-gray-200 rounded-xl px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
            <option value="">Todas as categorias</option>
            {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.emoji} {c.label}</option>)}
          </select>
          {!isCompany && (
            <select value={type} onChange={e => setType(e.target.value)} className="flex-1 border border-gray-200 rounded-xl px-3 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400">
      ...
      <div className="flex items-center justify-between pt-1">
        ...
        {!user && job.status === "OPEN" && (
          <button onClick={() => setPage("login")}
            className="flex items-center gap-1.5 text-teal-700 font-bold text-xs px-3 py-1.5 rounded-xl border border-teal-200 hover:bg-teal-50 transition-colors">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className={`w-full border rounded-xl pl-10 pr-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-teal-400 transition-colors
            ...
            <div>
              ...
              <textarea value={proposalForm.message} onChange={e => setProposalForm(p => ({ ...p, message: e.target.value }))}
                ...
                rows={4} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none" />
            </div>
            <div className="flex gap-3">
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center">
        <div className="bg-white border border-gray-200 rounded-2xl p-12 shadow-sm">
          ...
          <button onClick={() => setPage("services")} className="mt-6 bg-teal-700 text-white font-bold px-6 py-3 rounded-xl text-sm hover:bg-teal-800 transition-colors">Ver Vagas & Serviços</button>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-4 sm:py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm space-y-4">
        ...
        <button onClick={submit} disabled={onCooldown}
          className={`w-full font-bold py-4 rounded-xl text-base transition-colors shadow-sm ${onCooldown ? "bg-gray-300 text-gray-500 cursor-not-allowed" : "bg-teal-700 hover:bg-teal-800 text-white"}`}>
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-t border-gray-200 bg-white flex flex-col gap-1.5">
        ...
        <input
          ...
          className="flex-1 border border-gray-300 rounded-xl px-4 py-2.5 text-base focus:outline-none focus:ring-2 focus:ring-teal-400 bg-gray-50 resize-none"
          disabled={sending}
          maxLength={2000}
        />
        <button
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex bg-gray-100 rounded-xl p-1 gap-1 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-1.5 whitespace-nowrap px-2.5 sm:px-3 py-2 rounded-lg text-xs font-bold transition-all ${tab === t.key ? "bg-white shadow text-teal-700" : "text-gray-500"}`} style={{flexShrink:0}}>
            {t.label}
            {t.count !== null && t.count > 0 && <span className="bg-teal-100 text-teal-700 text-xs px-1.5 py-0.5 rounded-full">{t.count}</span>}
          </button>
        ))}
      </div>
                ...
                <div className="flex gap-2">
                  {p.status === "ACCEPTED" && (
                    <button onClick={() => { setCodeEntry({ proposal: p, job }); setEnteredCode(""); setCodeError(""); }}
                      className="flex-1 text-xs font-bold py-2.5 rounded-xl bg-teal-700 text-white hover:bg-teal-800 transition-colors">
      ...
      {tab === "profile" && (
        <div className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
          ...
          {editing ? (
            <div className="space-y-3">
              {[["Nome", "name", 100], ["WhatsApp", "phone", 20]].map(([l, k, max]) => (
                <div key={k}>
                  <label className="text-xs font-bold text-gray-600 block mb-1">{l}</label>
                  <input value={form[k]} onChange={e => setForm(p => ({ ...p, [k]: e.target.value }))} maxLength={max} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400" />
                </div>
              ))}
              <div>
                <label className="text-xs font-bold text-gray-600 block mb-1">Bairro</label>
              ...
              <div>
                ...
                <textarea value={form.bio} onChange={e => setForm(p => ({ ...p, bio: e.target.value }))} rows={3} maxLength={800} className="w-full border border-gray-300 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none" />
              </div>
              <div>
                <label className="text-xs font-bold text-gray-600 block mb-2">Áreas de atuação</label>
                <div className="flex flex-wrap gap-2">
      ...
      {codeEntry && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4 backdrop-blur-sm">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm">
            <div className="p-6">
              <div className="text-center mb-5">
                <div className="w-14 h-14 bg-teal-50 border-2 border-teal-200 rounded-full flex items-center justify-center mx-auto mb-3">
                  <span className="text-2xl">🔐</span>
                </div>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder="000000"
                  className="w-full text-center text-3xl font-black tracking-[0.4em] border-2 border-gray-300 rounded-2xl py-4 focus:outline-none focus:border-teal-500 focus:ring-2 focus:ring-teal-200 bg-gray-50"
              ...
              <div className="flex flex-col gap-3">
                <button
                  ...
                  className="w-full bg-teal-700 hover:bg-teal-800 disabled:bg-teal-300 text-white font-black py-4 rounded-xl transition-colors text-sm"
                >
      ...
      {rateClientTarget && (
        <Modal title="⭐ Avaliar contratante" onClose={() => setRateClientTarget(null)}>
            ...
            <div className="flex flex-wrap gap-2">
              {["Ótimo contratante!","Comunicação excelente","Pagamento em dia","Bem organizado","Atencioso e educado","Indicaria a outros"].map(t => (
                <button key={t} onClick={() => setClientReviewForm(p => ({...p, comment: t}))}
                  className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${clientReviewForm.comment === t ? "bg-teal-700 border-teal-700 text-white" : "border-gray-300 text-gray-600 hover:border-teal-400"}`}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <textarea value={clientReviewForm.comment} onChange={e => setClientReviewForm(p => ({...p, comment: e.target.value}))}
            ...
            rows={3} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none mb-4" />
          <div className="flex gap-3">
            ...
            }} className="flex-1 bg-teal-700 hover:bg-teal-800 disabled:bg-teal-300 text-white font-black py-3 rounded-xl text-sm transition-colors">
...
const CodeConfirmModal = ({ codeModal, onConfirmed, onCancel }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4 backdrop-blur-sm">
      <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm">
        <div className="p-6 text-center">
          <div className="w-14 h-14 bg-teal-50 border-2 border-teal-200 rounded-full flex items-center justify-center mx-auto mb-4">
          ...
          {waiting ? (
            <>
              ...
              <div className="bg-gray-50 border-2 border-teal-300 rounded-2xl py-5 mb-3">
                <p className="text-4xl font-black tracking-[0.3em] text-teal-700 select-all">
                  {codeModal.code}
                </p>
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={() => setPage("request-service")} className="flex items-center gap-2 bg-teal-700 hover:bg-teal-800 text-white font-bold px-4 py-3 rounded-xl text-sm transition-colors" style={{flexShrink:0}}>
          <I.Plus />Nova Vaga
        </button>
      </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-teal-50 border border-teal-200 rounded-2xl text-center" style={{padding:"10px 4px"}}>
          <div className="text-2xl font-black text-teal-700">{myJobs.length}</div>
          <div className="text-teal-600 font-semibold" style={{fontSize:"11px",marginTop:"2px"}}>Vagas</div>
        </div>
      ...
      <div className="flex bg-gray-100 rounded-xl p-1 gap-1 overflow-x-auto">
        {[["jobs", "📋 Vagas"], ["conversations", "🗨️ Conversas"]].map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex-1 whitespace-nowrap py-2 px-3 rounded-lg text-sm font-bold transition-all ${tab === key ? "bg-white shadow text-teal-700" : "text-gray-500"}`} style={{flexShrink:0}}>{label}</button>
        ))}
        {/* "Buscar Serviços" navega para a ServicesPage global */}
        <button onClick={() => setPage("services")}
          className="flex-1 whitespace-nowrap py-2 px-3 rounded-lg text-sm font-bold transition-all text-gray-500 hover:text-teal-700" style={{flexShrink:0}}>
          🔍 Buscar Serviços
        </button>
      </div>
              ...
              <div className="flex flex-wrap gap-2" style={{marginTop:"4px"}}>
                {(job.status === "OPEN" || job.status === "IN_PROGRESS") && accepted && (
                  <button onClick={() => markComplete(job)} disabled={codeGenerating}
                    className="flex-1 px-3 py-2 bg-teal-600 hover:bg-teal-700 disabled:bg-teal-300 text-white rounded-xl text-xs font-bold transition-colors text-center" style={{minWidth:"130px"}}>
                      ...
                      <div key={p.id} className={`border rounded-xl p-4 ${statusColors[p.status]}`}>
                        ...
                        <div className="flex items-center gap-3" style={{flexWrap:"wrap"}}>
                          ...
                          <div className="text-right" style={{flexShrink:0}}>
                            ...
                            {p.status === "ACCEPTED" && <span className="text-xs font-bold text-teal-600 bg-teal-100 rounded-full" style={{padding:"2px 8px"}}>✓ Aceita</span>}
                        ...
                        {p.status === "PENDING" && (
                          <div className="flex gap-2" style={{marginTop:"12px"}}>
                            <button onClick={() => acceptProposal(p.id, job.id)}
                              ...
                              className="flex-1 bg-teal-700 hover:bg-teal-800 disabled:bg-teal-300 text-white font-bold rounded-xl text-sm transition-colors" style={{padding:"10px"}}>
          ...
          <div className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {["Excelente profissional!","Trabalho de qualidade","Pontual e comprometido","Superou as expectativas","Comunicação excelente","Recomendo muito!"].map(t => (
                  <button key={t} onClick={() => setReviewForm(p => ({...p, comment: t}))}
                    className={`text-xs px-3 py-1.5 rounded-full border transition-colors ${reviewForm.comment === t ? "bg-teal-700 border-teal-700 text-white" : "border-gray-300 text-gray-600 hover:border-teal-400"}`}>
                    {t}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <textarea value={reviewForm.comment} onChange={e => setReviewForm(p => ({ ...p, comment: e.target.value }))}
                ...
                rows={3} className="w-full border border-gray-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-teal-400 resize-none" />
            </div>
            <div className="flex gap-3">
          ...
          <div key={job.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-3">
            ...
            <div className="flex flex-wrap gap-2" style={{paddingTop:"8px"}}>
              {(job.status === "IN_PROGRESS" || job.status === "OPEN") && accepted && (
                <button onClick={() => markComplete(job)} disabled={codeGenerating}
                  className="flex-1 px-3 py-2 bg-teal-600 hover:bg-teal-700 disabled:bg-teal-300 text-white rounded-xl text-xs font-bold transition-colors text-center" style={{minWidth:"140px"}}>
                  {codeGenerating ? "Gerando..." : "🔐 Gerar código de conclusão"}
                </button>
              )}
              {(job.status === "IN_PROGRESS" || job.status === "OPEN") && !accepted && (
...
const PrefeituraDashboard = () => {
  ...
  const StatCard = ({ icon, value, label, sub, color = "teal" }) => {
    const colors = { teal: "bg-teal-50 border-teal-200 text-teal-700", blue: "bg-blue-50 border-blue-200 text-blue-700", amber: "bg-amber-50 border-amber-200 text-amber-700", purple: "bg-purple-50 border-purple-200 text-purple-700", rose: "bg-rose-50 border-rose-200 text-rose-700", green: "bg-green-50 border-green-200 text-green-700" };
    return (
            ...
            <table className="w-full text-sm min-w-[480px]">
              ...
              <tbody className="divide-y divide-gray-50">
                {recentJobs.map(j => {
                  ...
                  const statusColor = j.status==="OPEN" ? "bg-teal-100 text-teal-700" : j.status==="IN_PROGRESS" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-500";
            ...
            <div className="space-y-3">
              {topPros.map((pro, i) => (
                <div key={pro.id} className="flex items-center gap-3">
                  ...
                  {pro.verified && <span className="text-xs bg-cyan-100 text-cyan-700 px-2 py-0.5 rounded-full font-bold">✓ Verificado</span>}
                </div>
              ))}
              ...
              <tbody className="divide-y divide-gray-50">
                {candidates.slice(0, 30).map(u => (
                  <tr key={u.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">{u.verified ? <span className="text-xs bg-cyan-100 text-cyan-700 px-2 py-0.5 rounded-full font-bold">✓</span> : <span className="text-xs text-gray-400">Pendente</span>}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {tab === "report" && (
        <div className="space-y-4">
          <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5">
            ...
            <p className="text-xs text-blue-700 mb-4">Dados consolidados da plataforma EmpregaFácil no município.</p>
...
const NotificationsPage = () => {
  ...
  const typeConfig = {
    message:           { icon: "💬", label: "Mensagem",          accent: "bg-teal-100 text-teal-700"  },
        ...
        {myNotifs.length > 0 && (
          <div className="flex gap-2">
            {unread > 0 && (
              <button
                onClick={markAllRead}
                className="text-xs font-semibold text-teal-600 hover:text-teal-800 bg-teal-50 hover:bg-teal-100 px-3 py-1.5 rounded-xl transition-colors">
                ✓ Marcar todas
              </button>
            )}
            <button
            ...
            return (
              <div
                ...
                className={`
                  ...
                  ${!n.read ? "border-teal-200 bg-teal-50/40" : "border-gray-200 hover:border-gray-300"}
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    {!n.read && (
                      <span className="w-2 h-2 bg-teal-500 rounded-full flex-shrink-0" title="Não lida" />
                    )}
                  </div>
                  {/* Mensagem */}
...
export default function EmpregaFacilV2() {
  ...
  // ── Realtime: ouvir novas mensagens e criar popup + notificação ──
  useEffect(() => {
    if (!user) return;
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        if (msg.sender_id === userRef.current?.id) return; // ignora próprias msgs
        // Verifica se a conversa é do usuário atual
        ...
        if (conv.client_id !== uid && conv.professional_id !== uid) return;
        // Atualiza mensagens locais
        setDb(prev => ({
          ...prev,
        ...
        // Popup de nova mensagem — inclui conversationId para deep link
        setMsgPopup({
          type: "message",
          title: msg.sender_name,
          text: msg.text,
          actionPage: "messages",
          conversationId: msg.conversation_id, // usado para abrir a conversa direto
        });
        setTimeout(() => setMsgPopup(null), 5000);
        // Cria notificação persistente
        try {
          // Link no formato "messages|conversationId" — interpretado pelo handleClick da NotificationsPage
          await dbService.createNotification(uid, "message", `💬 ${msg.sender_name}: ${msg.text.slice(0, 60)}`, `messages|${msg.conversation_id}`);
  ...
  // ── Realtime: ouvir novas propostas e notificar o dono da vaga ──
  useEffect(() => {
    if (!user) return;
    const channel = supabase
      .channel("proposals-realtime-" + user.id)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "proposals" }, async (payload) => {
        const prop = payload.new;
        const uid = userRef.current?.id;
        if (!uid) return;

        // Verifica se o job pertence ao usuário atual (ele é o dono da vaga)
        const { data: job } = await supabase
          .from("jobs")
          .select("company_id, title")
          .eq("id", prop.job_id)
          .single();
        if (!job || job.company_id !== uid) return;

        // Atualiza lista de propostas local
        const newProp = {
        ...
        setDb(prev => ({
          ...prev,
          proposals: prev.proposals.some(p => p.id === newProp.id)
            ? prev.proposals
            : [newProp, ...prev.proposals],
        }));

        // Popup de nova proposta
        setMsgPopup({
          type: "proposal",
          title: prop.candidate_name,
          text: `Nova proposta para "${job.title}" — ${prop.price}`,
          actionPage: "dashboard",
        });
        setTimeout(() => setMsgPopup(null), 6000);

        // Persiste notificação no banco (garante que aparece mesmo após reload)
        try {
          await dbService.createNotification(
            uid,
            "proposal",
            `📋 ${prop.candidate_name} enviou uma proposta para "${job.title}"`,
            "dashboard"
          );
          const notifications = await dbService.getNotifications(uid);
          setDb(prev => ({ ...prev, notifications }));
      ...
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "proposals" }, async (payload) => {
        const prop = payload.new;
        const uid = userRef.current?.id;
        if (!uid || prop.candidate_id !== uid) return;

        // Atualiza status local da proposta do próprio usuário
        setDb(prev => ({
          ...prev,
          proposals: prev.proposals.map(p => p.id === prop.id ? { ...p, status: prop.status } : p),
        }));

        // Popup + notificação persistente ao receber resultado da proposta
        if (prop.status === "ACCEPTED") {
          const { data: job } = await supabase.from("jobs").select("title").eq("id", prop.job_id).single();
          const jobTitle = job?.title || "o serviço";
          setMsgPopup({
            type: "proposal_accepted",
            title: "Proposta aceita! 🎉",
            text: `Sua proposta para "${jobTitle}" foi aceita!`,
            actionPage: "dashboard",
          });
          setTimeout(() => setMsgPopup(null), 7000);
          // Persiste no banco para aparecer na aba de notificações mesmo após reload
          try {
            await dbService.createNotification(
              uid,
              "proposal_accepted",
              `🎉 Sua proposta para "${jobTitle}" foi aceita! Verifique seu painel.`,
              "dashboard"
            );
          } catch (_) {}
          ...
          setMsgPopup({
            ...
            text: `Sua proposta para "${jobTitle}" não foi selecionada.`,
            actionPage: "dashboard",
          });
          setTimeout(() => setMsgPopup(null), 6000);
          // Persiste no banco
          try {
            await dbService.createNotification(
              uid,
              "proposal_rejected",
              `❌ Sua proposta para "${jobTitle}" não foi selecionada desta vez.`,
              "dashboard"
            );
          } catch (_) {}
        }

        // Recarrega notificações locais
        try {
          const notifications = await dbService.getNotifications(uid);
          setDb(prev => ({ ...prev, notifications }));
        } catch (_) {}
  ...
  // ── Realtime: ouvir mudanças de status em jobs (candidato vê botão de código sem reload) ──
  useEffect(() => {
    if (!user) return;
    const channel = supabase
      .channel("jobs-status-realtime-" + user.id)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "jobs" }, payload => {
        const j = payload.new;
        setDb(prev => ({
          ...prev,
          jobs: prev.jobs.map(job => job.id === j.id
            ? { ...job, status: j.status }
            : job
          ),
        }));
  ...
  // ── Realtime: canal dedicado para notificações do usuário ──
  // Garante que notificações criadas por outros usuários (aceite de proposta,
  // nova mensagem offline, etc.) apareçam em tempo real sem precisar recarregar.
  useEffect(() => {
    if (!user) return;
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        schema: "public",
        table: "notifications",
        filter: `user_id=eq.${user.id}`, // só notificações deste usuário
      }, (payload) => {
        const n = payload.new;
        // Normaliza para o formato local
        const newNotif = {
          id: n.id,
          userId: n.user_id,
          type: n.type || "info",
        ...
        setDb(prev => ({
          ...prev,
          notifications: prev.notifications.some(x => x.id === newNotif.id)
            ? prev.notifications
  ...
  // ── Realtime: ouvir novas avaliações e atualizar rating ──
  useEffect(() => {
    if (!user) return; // guard: só ativa com usuário logado
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "reviews" }, async (payload) => {
        ...
        // Recarrega reviews e profissionais para atualizar médias
        try {
          const [reviews, users] = await Promise.all([dbService.getReviews(), dbService.getProfessionals()]);
          setDb(prev => ({ ...prev, reviews, users }));
        } catch (_) {}
        // Se é o profissional sendo avaliado, notifica com popup
        if (userRef.current?.id === rev.target_id) {
          const uid = rev.target_id;
          setMsgPopup({
            type: "review",
            title: rev.reviewer_name,
            text: `Te avaliou com ${rev.rating} estrela${rev.rating !== 1 ? "s" : ""}! "${rev.comment?.slice(0, 50) || ""}"`,
            actionPage: "dashboard",
          });
          setTimeout(() => setMsgPopup(null), 6000);
          try {
            await dbService.createNotification(uid, "review", `⭐ ${rev.reviewer_name} te avaliou com ${rev.rating} estrelas!`, "dashboard");
            const notifications = await dbService.getNotifications(uid);
            setDb(prev => ({ ...prev, notifications }));
          } catch (_) {}
        }
      ...
      .subscribe();
    return () => supabase.removeChannel(channel);
  }, [user?.id]); // reativa quando usuário muda
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "SIGNED_IN" && session?.user) {
        // Só processa aqui se não estiver sendo tratado pelo LoginPage diretamente
        // (evita race condition onde setUser é chamado duas vezes)
        try {
          const { data: profile } = await supabase.from("profiles").select("*").eq("id", session.user.id).single();
          if (profile) {
            const normUser = normProfile({ ...profile, email: session.user.email });
            // Atualiza usuário no contexto (LoginPage também pode ter feito isso — é idempotente)
            setUser(normUser);
            loadUserData(normUser.id);
          }
          // Independente de ter perfil ou não, garante que authLoading seja false
          setAuthLoading(false);
        } catch (e) {
