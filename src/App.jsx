...
// ─── MESSAGES PAGE ────────────────────────────────────────────────────────────
const MessagesPage = () => {
  const { db, setDb, user, pendingConvId, setPendingConvId } = useApp();
  ...
  useEffect(() => {
    ...
    dbService.getConversations(user.id)
      .then(convs => {
        ...
        // Se há uma conversa pendente (vinda de popup ou notificação), abre automaticamente
        if (pendingConvId) {
          const target = convs.find(c => c.id === pendingConvId);
          if (target) {
            setSelectedConv(target);
            setPendingConvId(null); // limpa após usar
          }
        }
      })
              ...
              {topPros.map((pro, i) => (
                <div key={pro.id} className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-1 text-yellow-500 font-black text-sm">
                    ★ <span className="text-gray-700">{pro.rating.toFixed(2)}</span>
                  </div>
