...
const MsgPopup = ({ popup, onClose, onOpen }) => {
  ...
  return (
    <div className="fixed bottom-20 sm:bottom-6 right-4 z-50 max-w-xs w-full"
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-black text-gray-900">{isCompany ? "Buscar Profissionais" : "Vagas & Serviços"}</h1>
          <p className="text-gray-500 text-sm mt-0.5">
            {isCompany ? `${filtered.length} profissional${filtered.length !== 1 ? "is" : ""} disponível${filtered.length !== 1 ? "is" : ""}` : `${filtered.length} oportunidades disponíveis`}
          </p>
        </div>
      </div>
...
const CandidateDashboard = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      {/* Tabs — overflow-x-auto + py-2.5 para área de toque adequada */}
      <div className="flex bg-gray-100 rounded-xl p-1 gap-1 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex items-center gap-1.5 whitespace-nowrap px-3 py-2.5 rounded-lg text-xs sm:text-sm font-bold transition-all shrink-0 ${tab === t.key ? "bg-white shadow text-teal-700" : "text-gray-500"}`}>
            {t.label}
...
const CompanyDashboard = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      {/* Header — flex-wrap evita overflow em nomes longos no mobile */}
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <h1 className="text-xl font-black text-gray-900 truncate">{user.companyName}</h1>
          <div className="flex items-center gap-2 mt-1 flex-wrap"><Badge role="COMPANY" /><NivelBadge verified={user.verified} /></div>
        </div>
        <button onClick={() => setPage("request-service")} className="flex items-center gap-2 bg-teal-700 hover:bg-teal-800 text-white font-bold px-4 py-2.5 rounded-xl text-sm transition-colors shrink-0">
          <I.Plus />Nova Vaga
        </button>
      </div>
      ...
      {/* Stats — 3 colunas com padding reduzido e label legível */}
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-teal-50 border border-teal-200 rounded-2xl p-3 text-center">
          ...
          <div className="text-xs font-semibold text-teal-600 mt-0.5 leading-tight">Vagas</div>
        </div>
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-3 text-center">
          ...
          <div className="text-xs font-semibold text-blue-600 mt-0.5 leading-tight">Propostas</div>
        </div>
        <div className="bg-purple-50 border border-purple-200 rounded-2xl p-3 text-center">
          ...
          <div className="text-xs font-semibold text-purple-600 mt-0.5 leading-tight">Em andamento</div>
        </div>
      </div>
      ...
      <div className="flex bg-gray-100 rounded-xl p-1 gap-1 overflow-x-auto">
        {[["jobs", "📋 Vagas"], ["conversations", "🗨️ Conversas"]].map(([key, label]) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex-1 whitespace-nowrap py-2.5 px-3 rounded-lg text-sm font-bold transition-all shrink-0 ${tab === key ? "bg-white shadow text-teal-700" : "text-gray-500"}`}>{label}</button>
        ))}
        {/* "Buscar Serviços" navega para a ServicesPage global */}
        <button onClick={() => setPage("services")}
          className="flex-1 whitespace-nowrap py-2.5 px-3 rounded-lg text-sm font-bold transition-all shrink-0 text-gray-500 hover:text-teal-700">
          🔍 Buscar Serviços
        </button>
      </div>
        ...
        return (
          <div key={job.id} className="bg-white border border-gray-200 rounded-2xl shadow-sm overflow-hidden">
            <div className="p-4">
              ...
              {/* Botões de ação — linha inferior com flex-wrap para mobile */}
              <div className="flex flex-wrap gap-2">
                {job.status === "OPEN" && (
                  <button onClick={() => markComplete(job)}
                    className="flex-1 min-w-[120px] px-3 py-2 bg-teal-600 hover:bg-teal-700 text-white rounded-xl text-xs font-bold transition-colors text-center">
                    ✅ Concluir vaga
                  </button>
                )}
                ...
                {job.status === "CLOSED" && !alreadyReviewedJobIds.has(job.id) && accepted && (
                  ...
                  }} className="flex-1 min-w-[120px] px-3 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded-xl text-xs font-bold transition-colors text-center">
                    ⭐ Avaliar
                  </button>
                )}
                {job.status === "CLOSED" && alreadyReviewedJobIds.has(job.id) && (
                  <span className="text-xs text-gray-400 font-medium py-2">✓ Avaliado</span>
                )}
                ...
                }} className="flex-1 min-w-[90px] px-3 py-2 bg-white border border-red-200 text-red-500 hover:bg-red-50 rounded-xl text-xs font-bold transition-colors text-center">
                  🗑️ Apagar
                </button>
              </div>
            </div>
                  ...
                  {proposals.map(p => {
                    ...
                    return (
                      <div key={p.id} className={`border rounded-xl p-4 ${statusColors[p.status]}`}>
                        {/* Linha 1: avatar + nome + preço — flex-wrap para mobile */}
                        <div className="flex items-start gap-3 flex-wrap">
                          <div className="flex items-center gap-2 flex-1 min-w-0">
                            <Avatar user={pCandidate} size={8} />
                            <div className="min-w-0">
                              <p className="font-bold text-gray-900 text-sm truncate">{p.candidateName}</p>
                          ...
                          <div className="text-right shrink-0">
                            <p className="font-black text-teal-700 text-base">{p.price}</p>
                            {p.status === "ACCEPTED" && <span className="text-xs font-bold text-teal-600 bg-teal-100 px-2 py-0.5 rounded-full">✓ Aceita</span>}
                            {p.status === "REJECTED" && <span className="text-xs text-gray-400 bg-gray-100 px-2 py-0.5 rounded-full">Recusada</span>}
                            {p.status === "PENDING" && <span className="text-xs text-yellow-700 bg-yellow-100 px-2 py-0.5 rounded-full">Pendente</span>}
                          </div>
                        </div>
                        <p className="text-sm text-gray-600 mt-2 bg-white/60 rounded-lg p-2">{p.message}</p>
                        {p.status === "PENDING" && (
                          <div className="flex gap-2 mt-3">
                            <button onClick={() => acceptProposal(p.id, job.id)}
                              className="flex-1 bg-teal-700 text-white font-bold py-2.5 rounded-xl text-sm hover:bg-teal-800 transition-colors">
                              ✅ Aceitar proposta
                            </button>
                          </div>
                        )}
...
const ClientDashboard = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      {/* Header com flex-wrap para não transbordar no mobile */}
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="min-w-0">
          <h1 className="text-xl font-black text-gray-900">Olá, {user.name.split(" ")[0]}!</h1>
          <div className="mt-1"><Badge role="CLIENT" /></div>
        </div>
        <button onClick={() => setPage("request-service")} className="flex items-center gap-2 bg-teal-700 hover:bg-teal-800 text-white font-bold px-4 py-2.5 rounded-xl text-sm transition-colors shrink-0">
          <I.Plus />Novo Pedido
        </button>
      ...
      {tab === "requests" && (myRequests.length === 0 ? (
        ...
        return (
          <div key={job.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm space-y-3">
            ...
            {/* Botões de ação — flex-wrap para mobile */}
