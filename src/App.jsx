...
// ─── NAVBAR ───────────────────────────────────────────────────────────────────
const Navbar = () => {
  const { user, setUser, setPage, db, setDb, handleLogout } = useApp();
                          ...
                          return (
                            <div key={n.id}
                              onClick={async () => {
                                ...
                                if (n.link) { setPage(n.link); setShowNotif(false); }
                              }}
...
export default function EmpregaFacilV2() {
  ...
  const [msgPopup, setMsgPopup] = useState(null); // { senderName, text, conversationId }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        try {
          await dbService.createNotification(uid, "message", `💬 ${msg.sender_name}: ${msg.text.slice(0, 60)}`, "messages");
    ...
    <AppContext.Provider value={ctx}>
      <div className="min-h-screen bg-gray-50 font-sans pb-16 sm:pb-0">
        ...
        <MsgPopup
          ...
          onOpen={() => { if (msgPopup?.actionPage) setPage(msgPopup.actionPage); setMsgPopup(null); }}
        />
