...
const dbService = {
  ...
  async submitVerification(userId, documentUrl, selfieUrl) {
  ...
  async uploadVerificationFile(userId, file, type) {
    const ext = file.name.split(".").pop();
    const filePath = `${userId}/${type}.${ext}`;
    const { error } = await supabase.storage.from("verification-docs").upload(filePath, file, { upsert: true });
    if (error) throw error;
    // Retorna path privado (não URL pública)
    return filePath;
  },

  async getVerificationSignedUrl(filePath) {
    const { data, error } = await supabase.storage.from("verification-docs").createSignedUrl(filePath, 60 * 60); // 1h
    if (error) throw error;
...
const ServiceCard = ({ job }) => {
  ...
  const submitProposal = async () => {
    ...
    // 3. Criar conversa (falha silenciosa — não bloqueia proposta)
    try {
...
const LoginPage = () => {
  ...
  const handleReset = async () => {
    ...
    setResetError("");
    try {
      await supabase.auth.resetPasswordForEmail(resetEmail, { redirectTo: window.location.href });
      setResetSent(true);
    } catch (e) {
...
const VerificationPage = () => {
  ...
  const status = user.verificationStatus || "nao_solicitado";

  const handleFile = (file, type) => {
    if (!file) return;
    if (file.size > 10 * 1024 * 1024) { setError("Arquivo muito grande. Máximo 10MB."); return; }
    const preview = URL.createObjectURL(file);
    if (type === "doc") { setDocFile(file); setDocPreview(preview); }
    else { setSelfieFile(file); setSelfiePreview(preview); }
    setError("");
  };

  const submit = async () => {
    if (!docFile || !selfieFile) { setError("Envie os dois documentos."); return; }
    if (!lgpdConsent) { setError("Você precisa aceitar a política de privacidade para continuar."); return; }
    setLoading(true); setError("");
    try {
      // Upload documento (storage privado)
      const docPath = await dbService.uploadVerificationFile(user.id, docFile, "documento");
      const selfiePath = await dbService.uploadVerificationFile(user.id, selfieFile, "selfie");
      // Salvar caminhos (não URLs públicas) no perfil
      await dbService.submitVerification(user.id, docPath, selfiePath);
      // Atualizar estado local
      setUser(u => ({ ...u, verificationStatus: "pendente" }));
      setSuccess(true);
    } catch (e) {
      console.error("Erro na verificação:", e);
      // Fallback: tenta salvar só o status mesmo sem storage configurado
      try {
        await dbService.submitVerification(user.id, "pendente_upload", "pendente_upload");
        setUser(u => ({ ...u, verificationStatus: "pendente" }));
        setSuccess(true);
      } catch(e2) {
  ...
  if (success || status === "pendente") {
    return (
      <div className="max-w-lg mx-auto px-4 py-12 text-center">
        <div className="bg-white border border-gray-200 rounded-2xl p-10 shadow-sm">
          <div className="text-6xl mb-4">⏳</div>
          <h2 className="text-2xl font-black text-gray-900 mb-2">Documentos em análise!</h2>
          <p className="text-gray-500 text-sm mb-6 leading-relaxed">
            Recebemos seus documentos e nossa equipe irá analisá-los em até 48 horas. Você será notificado por email assim que sua conta for verificada.
          </p>
          <div className="bg-yellow-50 border border-yellow-200 rounded-xl px-4 py-3 text-xs text-yellow-700 font-medium mb-6">
            🔒 Seus documentos são armazenados com segurança e não são exibidos publicamente.
          </div>
          <button onClick={() => setPage("dashboard")} className="w-full bg-teal-700 hover:bg-teal-800 text-white font-bold py-3 rounded-xl text-sm transition-colors">
            Voltar ao painel
          </button>
        </div>
      </div>
  ...
  if (status === "aprovado") {
    return (
      <div className="max-w-lg mx-auto px-4 py-12 text-center">
        <div className="bg-white border border-gray-200 rounded-2xl p-10 shadow-sm">
          <div className="text-6xl mb-4">✅</div>
          <h2 className="text-2xl font-black text-gray-900 mb-2">Conta verificada!</h2>
          <p className="text-gray-500 text-sm mb-6">Sua identidade foi verificada com sucesso. Você tem acesso completo à plataforma.</p>
          <NivelBadge verified={true} />
          <div className="mt-6">
            <button onClick={() => setPage("dashboard")} className="w-full bg-teal-700 hover:bg-teal-800 text-white font-bold py-3 rounded-xl text-sm transition-colors">Ir ao painel</button>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-lg mx-auto px-4 py-12">
        <div className="bg-white border border-gray-200 rounded-2xl p-8 shadow-sm">
          <div className="text-center mb-6">
            ...
            <h2 className="text-xl font-black text-gray-900 mb-1">Verificação rejeitada</h2>
            <p className="text-red-600 text-sm font-medium">Seus documentos não foram aceitos. Por favor, envie novamente com fotos nítidas e legíveis.</p>
          </div>
          {/* re-envia */}
          <button onClick={() => setUser(u => ({ ...u, verificationStatus: "nao_solicitado" }))}
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="text-center mb-8">
        <div className="w-16 h-16 bg-teal-50 border-2 border-teal-200 rounded-2xl flex items-center justify-center mx-auto mb-3">
          <span className="text-3xl">🛡️</span>
        </div>
        <h1 className="text-2xl font-black text-gray-900">Verificar Identidade</h1>
        <p className="text-gray-500 text-sm mt-1">Envie seus documentos para desbloquear todos os recursos da plataforma</p>
      </div>
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 mb-6">
        <p className="text-sm font-black text-teal-800 mb-2">🔓 Com a verificação você pode:</p>
        <ul className="text-sm text-teal-700 space-y-1">
          {["Contratar profissionais", "Enviar propostas para serviços", "Publicar anúncios", "Receber o badge 🛡️ Verificado"].map(i => (
      ...
      <div className="space-y-5">
        {/* Upload Documento */}
        <div>
          <label className="text-sm font-black text-gray-700 block mb-2">📄 Documento (RG ou CNH) *</label>
          <div
            onClick={() => docRef.current?.click()}
            className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all ${docPreview ? "border-teal-400 bg-teal-50" : "border-gray-300 hover:border-teal-400 hover:bg-teal-50"}`}
          >
            {docPreview ? (
              <div className="relative">
                <img src={docPreview} alt="Documento" className="w-full h-40 object-contain rounded-lg" />
                <button onClick={e => { e.stopPropagation(); setDocFile(null); setDocPreview(null); }}
              ...
              <div className="py-4">
                ...
                <p className="text-sm font-semibold text-gray-600">Clique para selecionar</p>
                <p className="text-xs text-gray-400 mt-1">RG ou CNH · Frente e verso · JPG/PNG/PDF · máx. 10MB</p>
              </div>
            )}
          </div>
          <input ref={docRef} type="file" accept="image/*,.pdf" className="hidden" onChange={e => handleFile(e.target.files?.[0], "doc")} />
        </div>

        {/* Upload Selfie */}
        <div>
          <label className="text-sm font-black text-gray-700 block mb-2">🤳 Selfie segurando o documento *</label>
          <div
            onClick={() => selfieRef.current?.click()}
            className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all ${selfiePreview ? "border-teal-400 bg-teal-50" : "border-gray-300 hover:border-teal-400 hover:bg-teal-50"}`}
          >
            {selfiePreview ? (
              <div className="relative">
                <img src={selfiePreview} alt="Selfie" className="w-full h-40 object-contain rounded-lg" />
                <button onClick={e => { e.stopPropagation(); setSelfieFile(null); setSelfiePreview(null); }}
                  className="absolute top-1 right-1 bg-red-500 text-white rounded-full w-6 h-6 flex items-center justify-center text-xs font-bold">✕</button>
              </div>
            ) : (
              <div className="py-4">
                <div className="text-3xl mb-2">🤳</div>
                <p className="text-sm font-semibold text-gray-600">Clique para selecionar</p>
                <p className="text-xs text-gray-400 mt-1">Foto segurando o documento aberto · Rosto visível · JPG/PNG · máx. 10MB</p>
              </div>
            )}
          </div>
          <input ref={selfieRef} type="file" accept="image/*" className="hidden" onChange={e => handleFile(e.target.files?.[0], "selfie")} />
        </div>

        {/* LGPD Consent */}
        <div className={`border-2 rounded-xl p-4 cursor-pointer transition-all ${lgpdConsent ? "border-teal-400 bg-teal-50" : "border-gray-200"}`}
          onClick={() => setLgpdConsent(v => !v)}>
          <div className="flex items-start gap-3">
            ...
            <p className="text-xs text-gray-600 leading-relaxed">
              <span className="font-bold text-gray-800">Consentimento LGPD:</span> Autorizo o uso dos meus dados pessoais e documentos para verificação de identidade na plataforma EmpregaFácil, conforme a Lei Geral de Proteção de Dados (LGPD — Lei nº 13.709/2018). Os documentos serão usados exclusivamente para fins de verificação, armazenados com segurança e não serão compartilhados com terceiros.
            </p>
          </div>
        </div>
        ...
        <div className="bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-xs text-gray-500 flex items-start gap-2">
          ...
          <span>Seus documentos são transmitidos com criptografia SSL e armazenados em ambiente seguro. Não são visíveis publicamente nem compartilhados com outros usuários.</span>
        </div>

        <button onClick={submit} disabled={loading || !docFile || !selfieFile || !lgpdConsent}
          ...
          {loading ? "Enviando documentos..." : "📤 Enviar para verificação"}
        </button>
      </div>
    </div>
  );
            ...
            <div className="p-5">
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="flex flex-col gap-2 items-end shrink-0">
                  {/* Botão Marcar como concluído — só quando IN_PROGRESS */}
                  ...
                  <button onClick={async () => { if (window.confirm("Excluir este pedido?")) { try { await dbService.deleteJob(job.id); setDb(prev => ({ ...prev, jobs: prev.jobs.filter(j => j.id !== job.id) })); } catch(e) { console.error(e); } } }}
        ...
        return (
          <div key={job.id} className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm space-y-3">
            ...
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex flex-col gap-2 items-end shrink-0">
                {/* A) Botão só aparece quando IN_PROGRESS */}
                {job.status === "IN_PROGRESS" && (
                  <button
                ...
                {/* A) Botão de avaliação para jobs CLOSED com candidato aceito ainda não avaliado */}
                {job.status === "CLOSED" && !alreadyReviewedJobIds.has(job.id) && accepted && (
                  <button
                    onClick={() => {
                      setReviewForm({ rating: 5, comment: "" });
...
const AdminDashboard = () => {
  ...
  const tabs = ["overview", "verificacoes", "users", "jobs", "proposals", "reviews"];

  useEffect(() => {
    if (tab === "verificacoes") {
      setVerifyLoading(true);
      dbService.getPendingVerifications()
  ...
  const handleApprove = async (userId) => {
    try {
      ...
      setDb(prev => ({
        ...prev,
        users: prev.users.map(u => u.id === userId ? { ...u, verified: true, verificationStatus: "aprovado" } : u)
      }));
  ...
  const handleReject = async (userId) => {
    if (!window.confirm("Rejeitar esta verificação?")) return;
    try {
      await dbService.rejectVerification(userId);
      setPendingVerifications(prev => prev.filter(u => u.id !== userId));
      setDb(prev => ({
        ...prev,
        users: prev.users.map(u => u.id === userId ? { ...u, verified: false, verificationStatus: "rejeitado" } : u)
      }));
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex gap-1 overflow-x-auto bg-gray-100 rounded-xl p-1">
        {tabs.map(t => (
          <button key={t} onClick={() => setTab(t)} className={`px-4 py-2 rounded-lg text-sm font-bold whitespace-nowrap transition-all capitalize ${tab === t ? "bg-white shadow text-red-600" : "text-gray-500"}`}>
            {t === "verificacoes" ? `🛡️ Verificações${pendingVerifications.length > 0 ? ` (${pendingVerifications.length})` : ""}` : t}
          </button>
        ))}
      </div>
      ...
      {tab === "verificacoes" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-black text-gray-900">Verificações Pendentes</h2>
            <button onClick={() => { setVerifyLoading(true); dbService.getPendingVerifications().then(setPendingVerifications).finally(() => setVerifyLoading(false)); }}
              className="text-xs text-teal-600 font-bold hover:underline">↻ Atualizar</button>
          </div>
          {verifyLoading ? (
            <div className="flex items-center justify-center py-12"><div className="w-8 h-8 border-4 border-teal-200 border-t-teal-600 rounded-full animate-spin" /></div>
          ) : pendingVerifications.length === 0 ? (
            <div className="bg-white border border-gray-200 rounded-2xl p-12 text-center text-gray-400">
              <div className="text-4xl mb-3">✅</div>
              <p className="font-semibold text-gray-600">Nenhuma verificação pendente</p>
              <p className="text-sm mt-1">Todas as solicitações foram processadas.</p>
            </div>
          ) : pendingVerifications.map(u => (
            <div key={u.id} className="bg-white border border-amber-200 rounded-2xl p-5 shadow-sm">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-0.5">Cadastro: {u.createdAt?.slice(0, 10) || "—"} · {u.bairro || "—"}</p>
                  </div>
                </div>
                <div className="flex gap-2 flex-shrink-0">
                  <button onClick={() => handleApprove(u.id)}
                    className="bg-teal-700 hover:bg-teal-800 text-white font-bold px-4 py-2 rounded-xl text-sm transition-colors">
                    ✅ Aprovar
                  </button>
                  <button onClick={() => handleReject(u.id)}
                    className="bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 font-bold px-4 py-2 rounded-xl text-sm transition-colors">
                    ✕ Rejeitar
                  </button>
                </div>
              </div>
              {/* Documentos — links seguros via signed URL */}
              <div className="mt-4 flex gap-3 flex-wrap">
                {u.documentUrl && u.documentUrl !== "pendente_upload" && (
                  <button onClick={async () => {
                    try {
                      const url = await dbService.getVerificationSignedUrl(u.documentUrl);
                      window.open(url, "_blank");
                    } catch(e) {
                      alert("Erro ao carregar documento: " + e.message);
                    }
                  }}
                    className="flex items-center gap-2 bg-blue-50 border border-blue-200 text-blue-700 font-bold px-4 py-2 rounded-xl text-xs hover:bg-blue-100 transition-colors">
                    📄 Ver Documento (RG/CNH)
                  </button>
                )}
                {u.selfieUrl && u.selfieUrl !== "pendente_upload" && (
                  <button onClick={async () => {
                    try {
                      const url = await dbService.getVerificationSignedUrl(u.selfieUrl);
                      window.open(url, "_blank");
                    } catch(e) {
                      alert("Erro ao carregar selfie: " + e.message);
                    }
                  }}
                    className="flex items-center gap-2 bg-purple-50 border border-purple-200 text-purple-700 font-bold px-4 py-2 rounded-xl text-xs hover:bg-purple-100 transition-colors">
                    🤳 Ver Selfie
                  </button>
                )}
                {(!u.documentUrl || u.documentUrl === "pendente_upload") && (
                  <p className="text-xs text-gray-400 italic">Documentos aguardando upload ou bucket não configurado</p>
                )}
              </div>
            </div>
          ))}
