...
// ─── VERIFICATION PAGE ────────────────────────────────────────────────────────
// ─── DIDIT KYC CONFIG ─────────────────────────────────────────────────────────
// Preencha com o ID do Workflow criado em app.didit.me → Workflows
// CLIENT_ID e CLIENT_SECRET ficam apenas na Edge Function — nunca no frontend.
...
const VerificationPage = () => {
  ...
  const [kycUrl, setKycUrl] = useState(null); // URL pronta para o usuário tocar

  // Fase 1: busca a URL do Didit (sem redirecionar ainda)
  const prepareVerification = async () => {
    ...
    try {
      ...
      if (!data?.url) throw new Error("URL de verificação não retornada.");

      await dbService.saveKycSession(user.id, data.session_id);
      setUser(u => ({ ...u, verificationStatus: "pendente", kycSessionId: data.session_id }));
      sessionStorage.setItem("kyc_session_url", data.url);

      // Mostra o botão — o redirect acontece quando o usuário TOCA (nunca bloqueado)
      setKycUrl(data.url);
    } catch (e) {
      console.error("Erro ao iniciar KYC:", e);
      setError("Não foi possível iniciar a verificação. Tente novamente ou entre em contato com o suporte.");
    } finally {
      setLoading(false);
    }
  };

  // Detecta retorno do Didit via ?kyc_return=1 na URL e inicia polling
  useEffect(() => {
  ...
  // Polling: verifica a cada 3s se o webhook do Didit já atualizou o banco
  useEffect(() => {
    if (!polling) return;
    let attempts = 0;
    const interval = setInterval(async () => {
      ...
      if (attempts >= 40) { setPolling(false); clearInterval(interval); } // timeout ~2min
    }, 3000);
    return () => clearInterval(interval);
  }, [polling, user.id]);
  ...
  if (status === "aprovado") return (
    <div className="max-w-lg mx-auto px-4 py-12 text-center">
      <div className="bg-white border border-gray-200 rounded-2xl p-10 shadow-sm">
        ...
        <p className="text-gray-500 text-sm mb-5 leading-relaxed">Sua verificação foi aprovada automaticamente. Você agora tem acesso completo à plataforma.</p>
    ...
    <div className="max-w-lg mx-auto px-4 py-12">
      <div className="bg-white border border-red-200 rounded-2xl p-8 shadow-sm">
        <div className="text-center mb-5">
          ...
          <p className="text-sm text-gray-500 leading-relaxed">O sistema não conseguiu verificar sua identidade. Isso pode ocorrer por imagem desfocada, documento expirado ou iluminação inadequada.</p>
        </div>
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-xs text-amber-700 mb-5 space-y-1">
          <p className="font-bold">💡 Dicas para nova tentativa:</p>
          <p>• Use boa iluminação (evite flash direto)</p>
          <p>• Mantenha o documento sem reflexos</p>
          <p>• Rosto totalmente visível na selfie</p>
          <p>• Documento dentro da validade</p>
        </div>
        <button onClick={() => setUser(u => ({ ...u, verificationStatus: "nao_solicitado", kycSessionId: null }))}
  ...
  if (status === "pendente" || polling) return (
    <div className="max-w-lg mx-auto px-4 py-12 text-center">
      <div className="bg-white border border-gray-200 rounded-2xl p-10 shadow-sm">
        ...
        <h2 className="text-xl font-black text-gray-900 mb-2">{polling ? "Aguardando resultado..." : "Verificação em andamento"}</h2>
        <p className="text-gray-500 text-sm mb-4 leading-relaxed">
          {polling
            ? "O Didit está processando seus documentos. Isso leva apenas alguns segundos."
            : "Complete o processo de verificação na janela que abrimos. Retorne aqui ao finalizar."}
        </p>
        {!polling && (
          <div className="space-y-3">
            <button onClick={() => {
              const savedUrl = sessionStorage.getItem("kyc_session_url");
            ...
            <button onClick={() => setPolling(true)}
              className="w-full border border-teal-300 text-teal-700 font-bold py-3 rounded-xl text-sm hover:bg-teal-50 transition-colors">
              ↻ Já concluí — verificar resultado
            </button>
          </div>
        )}
        <button onClick={() => setPage("dashboard")} className="w-full text-gray-400 text-sm hover:text-gray-600 py-2 mt-3">Verificar mais tarde</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="bg-white border border-gray-200 rounded-2xl p-5 mb-5 shadow-sm space-y-4">
        ...
        {[
          { n: "1", icon: "📱", title: "Documento", desc: "Foto do seu RG ou CNH (frente e verso)" },
          { n: "2", icon: "🤳", title: "Selfie", desc: "Uma selfie para confirmar que é você" },
          { n: "3", icon: "⚡", title: "Resultado instantâneo", desc: "A IA processa e aprova em segundos" },
        ].map(s => (
      ...
      <div className="bg-teal-50 border border-teal-200 rounded-2xl p-4 mb-5">
        ...
        {["Contratar profissionais", "Enviar propostas para serviços", "Publicar pedidos de serviço", "Exibir o badge 🛡️ Verificado no perfil"].map(i => (
          <div key={i} className="flex items-center gap-2 text-sm text-teal-700 mb-1">
            <span className="text-teal-500 font-bold">✓</span>{i}
          </div>
        ))}
      </div>

      {/* Privacidade */}
      <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 text-xs text-gray-500 mb-5 space-y-1.5">
        <p className="font-bold text-gray-700">🔒 Privacidade e segurança</p>
        <p>• A verificação é feita pelo <strong>Didit</strong>, provedor especializado em KYC</p>
        <p>• <strong>Seus documentos nunca são armazenados no EmpregaFácil</strong></p>
        <p>• Recebemos apenas o resultado: Aprovado ou Recusado</p>
        <p>• Compatível com LGPD e GDPR · Criptografia ponta a ponta</p>
      </div>
      ...
      <div onClick={() => setLgpdConsent(v => !v)}
        ...
        <div className="flex items-start gap-3">
          ...
          <p className="text-xs text-gray-600 leading-relaxed">
            <span className="font-bold text-gray-800">Consentimento LGPD:</span> Autorizo o uso dos meus dados biométricos e documentos para verificação de identidade pelo provedor Didit, conforme a Lei Geral de Proteção de Dados (Lei nº 13.709/2018). Entendo que os documentos são processados pelo Didit e não ficam armazenados na plataforma EmpregaFácil.
          </p>
        </div>
      </div>

      {/* Fase 1: preparar — busca a URL */}
      {!kycUrl && (
        <button onClick={prepareVerification} disabled={loading || !lgpdConsent}
          ...
          {loading ? "Preparando verificação..." : "🚀 Iniciar verificação gratuita"}
        </button>
      )}

      {/* Fase 2: URL pronta — botão de toque direto (nunca bloqueado no mobile) */}
      {kycUrl && (
        <a href={kycUrl}
          className="w-full bg-teal-700 hover:bg-teal-800 text-white font-black py-4 rounded-xl text-base transition-colors flex items-center justify-center gap-2 no-underline">
          📷 Toque aqui para tirar as fotos
        </a>
      )}

      <p className="text-center text-xs text-gray-400 mt-3">Verificação automática por <strong>Didit KYC</strong> · Gratuito para você</p>
    </div>
  );
};
