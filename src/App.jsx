...
const ServiceCard = ({ job }) => {
  ...
  const submitProposal = async () => {
    ...
    const priceStr = String(proposalForm.price).replace(/[^0-9.,]/g, "").slice(0, 15);
    if (!priceStr || !message) { setError("Preencha todos os campos."); return; }
  ...
  const submit = async () => {
    ...
    try {
      const newJob = await dbService.createJob({
        ...form, title, description, companyId: user.id, companyName: sanitize(user.companyName || user.name).slice(0, 100),
      });
