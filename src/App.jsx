...
const MsgPopup = ({ popup, onClose, onOpen }) => {
  ...
  return (
    <div className="fixed bottom-20 sm:bottom-6 right-4 z-50 max-w-xs w-full animate-bounce-in">
      <div className="bg-gray-900 text-white rounded-2xl shadow-2xl p-4 flex items-start gap-3">
        <div className="w-9 h-9 bg-teal-500 rounded-full flex items-center justify-center text-sm font-black flex-shrink-0">
          {popup.senderName?.[0]?.toUpperCase() || "?"}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-bold text-teal-300 mb-0.5">{popup.senderName}</p>
          <p className="text-sm text-white leading-snug line-clamp-2">{popup.text}</p>
        </div>
        <div className="flex flex-col gap-1 flex-shrink-0">
          <button onClick={onOpen}
            className="text-xs bg-teal-500 hover:bg-teal-400 text-white font-bold px-2 py-1 rounded-lg transition-colors">
            Ver
          </button>
          <button onClick={onClose}
                ...
                {showNotif && (
                  <div className="absolute right-0 top-12 w-72 bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden z-50">
                    ...
                    {myNotifs.length === 0 ? (
                      ...
                      myNotifs.map(n => (
                        <div key={n.id}
                          onClick={() => { if (n.link) { setPage(n.link); setShowNotif(false); } }}
                          className={`px-4 py-3 text-sm border-b border-gray-50 ${!n.read ? "bg-teal-50" : ""} ${n.link ? "cursor-pointer hover:bg-gray-50" : ""}`}>
...
const CompanyDashboard = () => {
  ...
  const acceptProposal = async (proposalId, jobId) => {
    try {
      ...
      // Notificar o profissional que sua proposta foi aceita
      ...
      if (proposal?.candidateId) {
        try {
          await dbService.createNotification(
            ...
            "proposal",
            `🎉 Sua proposta para "${job?.title || "o serviço"}" foi aceita!`,
            "dashboard"
          );
        } catch (_) {}
      }
...
export default function EmpregaFacilV2() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes", {
        ...
        setMsgPopup({ senderName: msg.sender_name, text: msg.text, conversationId: msg.conversation_id });
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "reviews" }, async (payload) => {
        ...
        // Se é o profissional sendo avaliado, notifica
        if (userRef.current?.id === rev.target_id) {
          try {
            await dbService.createNotification(rev.target_id, "review", `⭐ ${rev.reviewer_name} te avaliou com ${rev.rating} estrelas!`);
            const notifications = await dbService.getNotifications(rev.target_id);
            setDb(prev => ({ ...prev, notifications }));
          } catch (_) {}
        }
      })
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "SIGNED_IN" && session?.user) {
        setUser(prev => { if (prev === null) return null; return prev; });
        try {
          const { data: profile } = await supabase.from("profiles").select("*").eq("id", session.user.id).single();
          if (profile) {
            const normUser = normProfile({ ...profile, email: session.user.email });
            setUser(u => { if (u === null) return null; return normUser; });
            loadUserData(normUser.id);
          }
  ...
  const bottomNav = user && [
    ...
    // Serviços aparece para CANDIDATE (buscando oportunidades) e COMPANY (buscando serviços)
    ...(user.role === "CANDIDATE" ? [{ key: "services", icon: <I.Briefcase />, label: "Serviços" }] : []),
    { key: "professionals", icon: <I.User />, label: "Profissionais" },
    ...(user.role === "COMPANY" ? [{ key: "services", icon: <I.Search />, label: "Buscar Serviços" }] : []),
    ...
    <AppContext.Provider value={ctx}>
      <div className="min-h-screen bg-gray-50 font-sans pb-16 sm:pb-0">
        ...
        <MsgPopup
          ...
          onOpen={() => { setPage("messages"); setMsgPopup(null); }}
        />
