  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="flex bg-gray-100 rounded-xl p-1 gap-1">
        {[["jobs", "📋 Vagas"], ["search", "🔍 Buscar Serviços"], ["conversations", "🗨️ Conversas"]].map(([key, label]) => (
      ...
      {tab === "search" && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-sm text-gray-500">{availableServices.length} serviço{availableServices.length !== 1 ? "s" : ""} disponível{availableServices.length !== 1 ? "is" : ""}</p>
          </div>
          {availableServices.length === 0 ? (
            <div className="bg-white border-2 border-dashed border-gray-200 rounded-2xl p-12 text-center text-gray-400">
              <div className="text-5xl mb-3">🔍</div>
              <p className="font-bold mb-1">Nenhum serviço disponível</p>
              <p className="text-sm">Profissionais ainda não publicaram ofertas de serviço.</p>
            </div>
          ) : (
            <div className="grid sm:grid-cols-2 gap-4">
              {availableServices.map(job => <ServiceCard key={job.id} job={job} />)}
            </div>
          )}
        </div>
      )}
