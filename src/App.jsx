...
const dbService = {
  ...
  // Profissional confirma com o código → marca proposta COMPLETED, job CLOSED
  async confirmCompletion(proposalId, jobId, code) {
    // Busca proposta e verifica código
    const { data: prop, error: fetchErr } = await supabase
      .from("proposals")
      .select("completion_code, candidate_id")
      .eq("id", proposalId)
      .single();
    if (fetchErr) throw fetchErr;
    if (!prop.completion_code || prop.completion_code !== code) {
      throw new Error("Código incorreto. Verifique com o contratante.");
    }
    // Marca proposta como concluída e limpa o código
    ...
    // Incrementa completed_jobs do profissional
    try { await supabase.rpc("increment_completed_jobs", { p_id: prop.candidate_id }); } catch (_) {}
  },
...
const LoginPage = () => {
  ...
  const handle = async e => {
    ...
    try {
      ...
      loginRateLimiter.reset(form.email.toLowerCase()); // limpa contador após sucesso
      ...
      if (profile) {
        ...
        // Perfil não existe — pode ser conta recém-criada aguardando confirmação de email
        setError("Perfil não encontrado. Confirme seu email ou entre em contato com o suporte.");
      }
      // setLoading(false) já é chamado no finally — não precisa aqui
    } catch (e) {
              ...
              <div className="flex flex-col gap-3">
                <button
                  onClick={async () => {
                    ...
                    setCodeError("");
                    try {
                      await dbService.confirmCompletion(codeEntry.proposal.id, codeEntry.proposal.jobId, enteredCode);
                      setDb(prev => ({
                        ...prev,
...
const CompanyDashboard = () => {
  ...
  // Gerar código de conclusão → exibe modal com código para o contratante passar ao profissional
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div style={{minWidth:0, flex:"1 1 auto"}}>
          <h1 className="text-xl font-black text-gray-900" style={{wordBreak:"break-word"}}>{user.companyName}</h1>
                        ...
                        {p.status === "PENDING" && (
                          <div className="flex gap-2" style={{marginTop:"12px"}}>
                            <button onClick={() => acceptProposal(p.id, job.id)}
                              className="flex-1 bg-teal-700 text-white font-bold rounded-xl text-sm hover:bg-teal-800 transition-colors" style={{padding:"10px"}}>
                              ✅ Aceitar proposta
                            </button>
                          </div>
                        )}
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 p-0 sm:p-4 backdrop-blur-sm">
          <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm">
            <div className="p-6 text-center">
              ...
              <h3 className="font-black text-gray-900 text-lg mb-1">Código de conclusão</h3>
              <p className="text-sm text-gray-500 mb-5">
                Mostre este código para <strong>{codeModal.proposal.candidateName}</strong>.<br/>
                Ele precisa inserir no painel para confirmar a conclusão.
              </p>
              {/* Código em destaque */}
              <div className="bg-gray-50 border-2 border-teal-300 rounded-2xl py-5 mb-5">
              ...
              <p className="text-xs text-gray-400 bg-amber-50 border border-amber-200 rounded-xl p-3 mb-5">
                ⚠️ O código expira quando o profissional confirmar. Não compartilhe com terceiros.
              </p>
              <div className="flex flex-col gap-3">
                <button
                  onClick={() => {
                    // Polling: verifica se o job foi fechado pelo profissional
                    const poll = setInterval(async () => {
                      const { data } = await supabase.from("jobs").select("status").eq("id", codeModal.job.id).single();
                      if (data?.status === "CLOSED") {
                        clearInterval(poll);
                        onCodeConfirmed(codeModal.job, codeModal.proposal);
                      }
                    }, 2000);
                    // Fallback: para o polling depois de 5 min
                    setTimeout(() => clearInterval(poll), 300_000);
                  }}
                  className="w-full bg-teal-700 hover:bg-teal-800 text-white font-black py-4 rounded-xl transition-colors text-sm"
                >
