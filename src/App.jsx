...
const CodeConfirmModal = ({ codeModal, onConfirmed, onCancel }) => {
  ...
  useEffect(() => {
    if (!codeModal?.job?.id) return;

    // 1. Realtime: ouve UPDATE no job para detectar status = CLOSED
    const channel = supabase
      .channel("job-close-" + codeModal.job.id)
      .on("postgres_changes", {
        event: "UPDATE",
        schema: "public",
        table: "jobs",
        filter: `id=eq.${codeModal.job.id}`,
      }, payload => {
    ...
    // 2. Polling como fallback a cada 3s (caso Realtime falhe)
    const poll = setInterval(async () => {
      const { data } = await supabase
        .from("jobs")
        .select("status")
        .eq("id", codeModal.job.id)
        .single();
      ...
      <div className="bg-white rounded-t-3xl sm:rounded-2xl shadow-2xl w-full sm:max-w-sm">
        <div className="p-6 text-center">
          ...
          {waiting ? (
            ...
            <p className="text-sm text-gray-500">O modal fechará em instantes...</p>
          )}
        </div>
      </div>
    </div>
