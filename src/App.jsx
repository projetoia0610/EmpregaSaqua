...
export default function EmpregaFacilV2() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      ...
      if (event === "SIGNED_IN" && session?.user) {
        // Não redirecionar se o usuário acabou de fazer logout (user já foi limpo)
        setUser(prev => {
          if (prev === null) return null; // logout em andamento, ignorar
          return prev;
        });
        try {
          const { data: profile } = await supabase.from("profiles").select("*").eq("id", session.user.id).single();
          if (profile) {
            const normUser = normProfile({ ...profile, email: session.user.email });
            setUser(u => {
              // Só atualiza se ainda há uma sessão válida (não foi feito logout)
  ...
  const handleLogout = async () => {
    setUser(null);
    setPage("home");
    setDb(prev => ({ ...prev, messages: [], conversations: [] }));
